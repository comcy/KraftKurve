...
CT_IP_ACTUAL="$(pct exec "${CT_ID}" -- bash -c \
  "hostname -I 2>/dev/null | awk '{print \$1}'" 2>/dev/null || echo "unbekannt")"

# Kurz prüfen ob Container-Dienste laufen
RUNNING_CONTAINERS="$(pct exec "${CT_ID}" -- docker ps --format '{{.Names}}' 2>/dev/null | tr '\n' ' ' || echo "")"

echo ""
echo -e "  ${GN}${BOLD}╔══════════════════════════════════════════╗${CL}"
echo -e "  ${GN}${BOLD}║   ✔  INSTALLATION ERFOLGREICH            ║${CL}"
echo -e "  ${GN}${BOLD}╚══════════════════════════════════════════╝${CL}"
echo ""
...
printf "  %-22s %s\n"  "Storage:"        "$CT_STORAGE  (${CT_DISK}GB)"
printf "  %-22s %s\n"  "Ressourcen:"     "${CT_RAM}MB RAM  ${CT_CORES} Cores"
printf "  %-22s %s\n"  "Docker-Dienste:" "${RUNNING_CONTAINERS:-keine gefunden}"
echo ""
echo -e "  ${BOLD}── Nächste Schritte ────────────────────────${CL}"
echo ""
echo -e "  ${YW}HTTPS empfohlen${CL} (PWA-Install + Offline-Mode benötigt HTTPS):"
echo -e "  ${DIM}  Nginx Proxy Manager → Proxy Host → ${CT_IP_ACTUAL}:${APP_PORT} → Let's Encrypt${CL}"
echo ""
echo -e "  ${BOLD}── Wartung ─────────────────────────────────${CL}"
echo ""
...
printf "  %-22s %s\n"  "Daten:"   "${APP_DIR}/data/  (im Container)"
echo ""
msg_line
echo ""
