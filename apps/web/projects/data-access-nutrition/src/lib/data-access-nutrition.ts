...
export interface ProteinGoalSettingsDto {
  proteinGoalG: number | null;
}
...
export class NutritionService {
  ...
  async getProteinGoal(): Promise<ProteinGoalSettingsDto> {
    try {
      return await firstValueFrom(this.http.get<ProteinGoalSettingsDto>(`${this.apiBase}/goal`));
    } catch (error) {
      throw this.toError(error);
    }
  }

  async setProteinGoal(proteinGoalG: number | null): Promise<ProteinGoalSettingsDto> {
    return this.executeWrite(
      this.makeOperation('PUT', `${this.apiBase}/goal`, { proteinGoalG }),
      async (operation) => {
        return await firstValueFrom(
          this.http.put<ProteinGoalSettingsDto>(
            `${this.apiBase}/goal`,
            { proteinGoalG },
            { headers: this.idempotencyHeaders(operation.id) },
          ),
